...
from .sandbox_tools import (
    ...
    ensure_sandbox_ocr_ready,
    execute,
    prepare_sandbox_ocr_runtime,
    sandbox_ls,
    sandbox_read,
...
## 소스 타입별 파싱 가이드
- PDF:
  - 먼저 pdfplumber 또는 pypdf로 1페이지 샘플을 추출해 텍스트 레이어 품질을 점검하세요.
  - 다음 조건 중 하나라도 만족하면 이미지 OCR 경로로 전환하세요.
    - 추출 텍스트 길이가 120자 미만
    - 한글/영문/숫자 토큰 비율이 30% 미만
    - 한 글자 토큰 비율이 40% 초과
    - 순서도/도식/프로세스 박스가 보이는데 텍스트 레이어가 충분하지 않음
  - 이미지 OCR 기본 경로:
    1. fitz(PyMuPDF)로 각 페이지를 300 DPI PNG로 렌더링
    2. `tesseract <image> stdout -l kor+eng --psm 6` 실행
    3. OCR 결과를 `/workspace/output/_ocr_{소스명}.md` 또는 page-level txt/md 파일로 저장
  - 샌드박스에는 `fitz`, `pdfplumber`, `pypdf`, `Pillow`, `pytesseract`, `tesseract`, `ocrmypdf`, `ghostscript`, `qpdf`가 준비되어 있다고 가정하고 활용하세요.
  - pdfplumber/pypdf는 probe 또는 품질이 명확히 좋은 텍스트 레이어를 읽을 때만 기본 경로로 사용하세요.
  - 순서도/도식은 prose 요약만 하지 말고, 노드 라벨 / 화살표 / 연결 관계 / edge label을 구조적으로 보존하세요.
  - Tesseract OCR 후에도 품질이 낮으면 원문에 없는 내용을 추정하지 말고 한계를 명시하세요.
...
### 노드 속성 규칙 (매우 중요)
- **모든 노드에 반드시 `name` 속성 필수**: `name`은 그래프에서 노드를 표시하는 기본 레이블입니다.
  - `title`이 아닌 **`name`**을 사용하세요. `name`이 없으면 그래프에서 UUID가 표시됩니다.
  - 예: `{"name": "보험계약대출", "content": "..."}`
  - 예: `{"name": "제33조 보험계약대출", "number": 33, "content": "..."}`
- **제목에서 불필요한 메타정보 제거**: 줄 끝의 페이지 번호, 불필요한 공백
  - 정규식: `re.sub(r'\\s+\\d+\\s*$', '', name).strip()`
- **구조 번호가 있으면 number 속성으로 분리 저장**: `{"name": "보험계약대출", "number": 33}`
...
def _list_sandbox_output_files(session_id: str) -> list[str]:
    """List files in /workspace/output/ inside the sandbox container."""

    try:
        return list_output_filenames(session_id)
    except Exception:
        pass
    return []
...
def _compose_continuation_prompt(
    ...
    output_files = _list_sandbox_output_files(session_id)
    if output_files:
        sections.append(
    ...
    if "_search_hints.json" in output_files:
        try:
            with sandbox_session_context(session_id):
...
def _create_build_agent():
    ...
    from .history_compression import HistoryCompressionMiddleware
...
def warm_up_agent() -> None:
    ...
    try:
        ensure_sandbox_ready()
        ocr_bootstrap_performed = False
        try:
            ensure_sandbox_ocr_ready()
        except Exception as exc:
            ocr_bootstrap_performed = True
            SmartLogger.log(
                "WARNING",
                "DeepAgent sandbox OCR runtime missing; attempting bootstrap",
            ...
            bootstrap_output = prepare_sandbox_ocr_runtime()
            ensure_sandbox_ocr_ready()
            SmartLogger.log(
                "INFO",
                "DeepAgent sandbox OCR runtime bootstrap completed",
                category="backend_status",
                params={
                    "phase": "startup",
                    "container_name": settings.container_name,
                    "sandbox_workdir": settings.sandbox_workdir,
                    "bootstrap_output": bootstrap_output[:1000],
                },
            )
    except Exception as exc:
        SmartLogger.log(
    ...
    SmartLogger.log(
        ...
        params={
            ...
            "container_name": settings.container_name,
            "sandbox_workdir": settings.sandbox_workdir,
            "ocr_bootstrap_performed": ocr_bootstrap_performed,
        },
    )
    get_agent("build")
    get_agent("answer")
...
def _describe_tool_call(tool_name: str, args: dict[str, Any] | None) -> str:
    ...
    if tool_name == "execute":
        ...
        if "tesseract" in lowered:
            return "렌더링된 페이지에 Tesseract OCR을 실행합니다"
        if "fitz" in lowered or "pymupdf" in lowered:
            return "PDF 페이지를 이미지로 렌더링합니다"
        if "ocrmypdf" in lowered:
            return "PDF에 OCR 텍스트 레이어를 생성합니다"
...
def _emit_tool_side_effects(
    ...
    # When a schema group is created, notify frontend with the schema name
    if tool_name == "schema_group_create":
        try:
            data = json.loads(content)
            schema_name = data.get("name", "")
            if schema_name:
                events.append(_format_sse("schema_created", {"schema_name": schema_name}))
...
def _run_agent_in_thread(
    ...
    context = sandbox_session_context(session_id)
    context.__enter__()
    try:
...
async def generate_sse(
    ...
    all_files = list_output_filenames(session_id) or generated_files
